    ...
    public abstract class MessagingEntityCache<T> where T : class, ICachedMessagingEntity
    {
            ...
            try
            {
                if (_entries.ContainsKey(path) == false)
                {
                    ...
                    entry = new CacheEntry<T>()
                    {
                        ...
                        Entity = await CreateEntity(logger, path),
                        ClosePending = false,
                        Path = path,
                    };
                ...
                // if this entity previously was closed, we need to create a new instance, otherwise return existing instance
                ...
                entry.Entity = await CreateEntity(logger, path);
        ...
        public async Task Release(ILogger logger, string path)
        {
            ...
            try
            {
                ...
                {
                    await CloseEntity(logger, entry, path);
                }
            }
            finally
            {
        ...
        public async Task Shutdown(ILogger logger)
        {
            ...
            try
            {
                ...
                foreach (var key in _entries.Keys)
                {
                    ...
                    await CloseEntity(logger, entry, key);
                }
            }
            finally
            {
